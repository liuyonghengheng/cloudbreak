...
import static com.sequenceiq.freeipa.client.FreeIpaClient.DEFAULT_BATCH_CALL_PARTITION_SIZE;
...
public class UserSyncService {
    ...
    private void retrySyncIfBatchCallHasWarnings(Stack stack, UmsUsersState umsUsersState, boolean fullSync, Multimap<String, String> warnings,
        ...
        if (fullSync && !warnings.isEmpty() && fmsToFreeipaBatchCallEnabled) {
            ...
            if (usersStateDifferenceChanged(usersStateDifferenceBeforeSync, usersStateDifferenceAfterSync)) {
                ...
                warnings.clear();
                applyDifference(umsUsersState, stack.getEnvironmentCrn(), warnings, usersStateDifferenceAfterSync, freeIpaClient, fmsToFreeipaBatchCallEnabled);
            }
        }
    }
    ...
    void addGroups(boolean fmsToFreeipaBatchCallEnabled, FreeIpaClient freeIpaClient, Set<FmsGroup> fmsGroups,
        ...
        List<AbstractFreeipaOperation> operations = Lists.newArrayList();
        for (FmsGroup fmsGroup : fmsGroups) {
            operations.add(GroupAddOperation.create(fmsGroup.getName(), warnings));
        }
        invokeOperation(operations, fmsToFreeipaBatchCallEnabled, freeIpaClient, DEFAULT_BATCH_CALL_PARTITION_SIZE,
                warnings, Set.of(FreeIpaErrorCodes.DUPLICATE_ENTRY), false);
    }
    ...
    void addUsers(boolean fmsToFreeipaBatchCallEnabled, FreeIpaClient freeIpaClient, Set<FmsUser> fmsUsers,
        ...
        invokeOperation(operations, fmsToFreeipaBatchCallEnabled, freeIpaClient, DEFAULT_BATCH_CALL_PARTITION_SIZE,
                warnings, Set.of(FreeIpaErrorCodes.DUPLICATE_ENTRY), true);
    }
    ...
    void removeUsers(boolean fmsToFreeipaBatchCallEnabled, FreeIpaClient freeIpaClient, Set<String> fmsUsers,
        ...
        invokeOperation(operations, fmsToFreeipaBatchCallEnabled, freeIpaClient, DEFAULT_BATCH_CALL_PARTITION_SIZE,
                warnings, Set.of(FreeIpaErrorCodes.NOT_FOUND), true);
    }
    ...
    void removeGroups(boolean fmsToFreeipaBatchCallEnabled, FreeIpaClient freeIpaClient, Set<FmsGroup> fmsGroups,
        ...
        invokeOperation(operations, fmsToFreeipaBatchCallEnabled, freeIpaClient, DEFAULT_BATCH_CALL_PARTITION_SIZE,
                warnings, Set.of(FreeIpaErrorCodes.NOT_FOUND), false);
    }
    ...
    void addUsersToGroups(boolean fmsToFreeipaBatchCallEnabled, FreeIpaClient freeIpaClient, Multimap<String, String> groupMapping,
        ...
        invokeOperation(operations, fmsToFreeipaBatchCallEnabled, freeIpaClient, BATCH_ADD_REMOVE_MEMBER_PARTITION_SIZE,
                warnings, Set.of(), false);
    }
    ...
    private void invokeOperation(List<AbstractFreeipaOperation> operations, boolean fmsToFreeipaBatchCallEnabled, FreeIpaClient freeipaClient,
            Integer partitionSize, BiConsumer<String, String> warnings, Set<FreeIpaErrorCodes> acceptableErrorCodes, boolean localErrorHandling)
            throws FreeIpaClientException {
        if (fmsToFreeipaBatchCallEnabled) {
            ...
            for (AbstractFreeipaOperation operation : operations) {
                try {
    ...
    private void singleOperationErrorHandling(FreeIpaClient freeipaClient, BiConsumer<String, String> warnings, Set<FreeIpaErrorCodes> acceptableErrorCodes,
        ...
        if (localErrorHandling) {
            if (FreeIpaClientExceptionUtil.isExceptionWithErrorCode(e, acceptableErrorCodes)) {
                LOGGER.debug(String.format("Operation %s failed with acceptable error: %s", operation.getMethodName(), e.getMessage()));
            } else {
                LOGGER.warn(e.getMessage());
                warnings.accept(String.format("operation %s failed", operation.getMethodName()), e.getMessage());
