...
public interface ImageCatalogRepository extends WorkspaceResourceRepository<ImageCatalog, Long> {
    ...
    @Query("SELECT new com.sequenceiq.authorization.resource.AuthorizationResource(i.id, i.resourceCrn) FROM ImageCatalog i " +
            "WHERE i.workspace.id = :workspaceId AND i.archived = false")
    List<AuthorizationResource> findAsAuthorizationResourcesInWorksapce(@Param("workspaceId") Long workspaceId);
